...
class Demo {
    ...
    private Demo(OndertekenenClient ondertekenenClient) {
        ...
        Transaction transaction = ondertekenenClient.createTransaction(Helper.createDemoTransaction(Helper.createDemoSigners()));
        ...
        ondertekenenClient.startTransaction(transaction.getId());

        /* Wait until the user has performed an action on the document */
        transaction = blockUntilUserAction(ondertekenenClient, transaction);

        /* The user has either Signed or Rejected the transaction! Get the signed document + receipt. */
        LOGGER.info("Downloading proof.");
        if (TransactionStatus.REJECTED != transaction.getStatus()) { /* Can't download a non existing file */
            Document signedDocument = ondertekenenClient.getSignedDocument(transaction.getId(), "Input.pdf", true);
     ...
     */
    Transaction blockUntilUserAction(OndertekenenClient ondertekenenClient, Transaction transaction) {
